...
(function ($) {
	...
	Fancybox.bind('[data-fancybox]', {
		Carousel: {
			...
			Zoomable: {
				Panzoom: {
					panMode: "drag", // "mousemove" or "drag"
					maxScale: 1,
					wheelAction: false,
				},
			},
	...
	$ttContactForm.submit(function(e) {
		...
		if ($(this).data("form-disabled") === true || $(this).attr("data-form-disabled") === "true") {
			$ttContactForm.addClass("cfm-submitted");
			$cfmContainerInner.empty().html('<span class="tt-cfm-error">Live form submission is not enabled yet. Please email Jason directly at <a href="mailto:jasons@elevateconstructionist.com">jasons@elevateconstructionist.com</a> or call <a href="tel:+16025718987">+1 (602) 571-8987</a>.</span>');
			scrollToContactFormTop();
			return;
		}

		const formEndpoint = $(this).data("form-endpoint");
		if (!formEndpoint) {
			$ttContactForm.addClass("cfm-submitted");
			$cfmContainerInner.empty().html('<span class="tt-cfm-error">This form is not configured yet. Please use the direct contact details on the page.</span>');
			scrollToContactFormTop();
			return;
		}
		...
		$.ajax({
			...
			if (response.success) {
				...
				$cfmContainerInner.html('<span class="tt-cfm-success">' + response.message + '</span>');
				...
				$cfmContainerInner.html('<span class="tt-cfm-error">' + response.message + '</span>');
			}
